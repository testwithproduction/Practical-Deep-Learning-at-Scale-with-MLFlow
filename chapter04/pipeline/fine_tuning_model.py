...
@click.command(
    help="This program finetunes a deep learning model for sentimental classification."
)
@click.option(
    "--foundation_model",
...
@click.option(
    "--fine_tuning_strategy", default="freeze", help="This is the finetuning strategy."
)
...
def task(foundation_model, fine_tuning_strategy, data_path):
    ...
    with mlflow.start_run(run_name="chapter04") as dl_model_tracking_run:
        trainer.finetune(
            classifier_model, datamodule=datamodule, strategy=fine_tuning_strategy
        )
        # trainer.test()
