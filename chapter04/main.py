...
_steps = ["download_data", "fine_tuning_model", "register_model"]
...
def run_pipeline(steps):
    ...
    with mlflow.start_run(run_name="pipeline", nested=True) as active_run:
        if "download_data" in active_steps:
            ...
            logger.info(
                "downloaded data is located locally in folder: %s", file_path_uri
            )
            logger.info(download_run)

        if "fine_tuning_model" in active_steps:
            fine_tuning_run = mlflow.run(
                ".", "fine_tuning_model", parameters={"data_path": file_path_uri}
            )
        ...
        if "register_model" in active_steps:
            if fine_tuning_run_id is not None and fine_tuning_run_id != "None":
                register_model_run = mlflow.run(
                    ".",
                    "register_model",
                    parameters={"mlflow_run_id": fine_tuning_run_id},
                )
                register_model_run = mlflow.tracking.MlflowClient().get_run(
                    register_model_run.run_id
                )
                logger.info(register_model_run)
            else:
                logger.info("no model to register since no trained model run id.")
    logger.info(
        "finished mlflow pipeline run with a run_id = %s", active_run.info.run_id
    )
